...
model.load_state_dict(torch.load("/root/autodl-tmp/02/bert_news_cls_epoch_50.pth"))
model.to(device)
model.eval()

# 读取待预测的数据
test_df = pd.read_csv(
    "/root/newstextclassification_new/data/test_a_processed.csv", sep="\t"
)
...
submission.to_csv(
    "/root/newstextclassification_new/results/news_submission_50.csv", index=False
)
